...
class BMCContainer():
    ...
    def b_import(self, fname):
        ...
        if self.bdat[:len(self.BIN_FILE_HEADER)] == self.BIN_FILE_HEADER:
            self.b_log(sys.stdout, True, 2, "Subsequent header version: %d." % (
        ...
        self.b_log(sys.stdout, True, 0,
                   "Successfully loaded '%s' as a %s container." % (self.fname, self.btype.decode()))
        return True
        ...
        while len(self.bdat) > 0:
            ...
            if self.btype == self.BIN_CONTAINER:
                ...
                if t_params & 0x08:  # This bit is always ONE when relevant data is smaller than expected data, thus it is most likely the "compression" bit flag.
                    ...
                    if cf == 4:
                        t_bmp = self.b_parse_rgb32b(
                            self.bdat[len(t_hdr):len(t_hdr) + cf * t_width * t_height].tobytes())
                        if t_height != 64:
                            old = True
                            o_bmp = self.b_parse_rgb32b(
                                self.bdat[len(t_hdr) + cf * t_width * t_height:len(t_hdr) + cf * 64 * 64].tobytes())
                    elif cf == 3:
                        t_bmp = self.b_parse_rgb24b(
                            self.bdat[len(t_hdr):len(t_hdr) + cf * t_width * t_height].tobytes())
                        if t_height != 64:
                            old = True
                        ...
                        if t_height != 64:
                            old = True
                            o_bmp = self.PALETTE + self.bdat[len(t_hdr) + cf * t_width * t_height:len(
                                t_hdr) + cf * 64 * 64].tobytes()
                    else:
                        self.b_log(sys.stderr, False, 3,
                                   "Unexpected bpp (%d) found during processing; aborting." % (8 * cf))
                        return False
                    bl = cf * 64 * 64
            if len(t_bmp) > 0:
    ...
    def b_export(self, dname):
        ...
        for i in range(len(self.bmps)):
            self.b_write(os.path.join(dname, "%s_%04d.bmp" % (self.fname, i)),
                         self.b_export_bmp(64, len(self.bmps[i]) // 256, self.bmps[i]))
            if self.oldsave and i < len(self.o_bmps) and len(self.o_bmps[i]) > 0:
                self.b_write(os.path.join(dname, "%s_old_%04d.bmp" % (self.fname, i)),
...
if __name__ == "__main__":
    ...
    prs.add_argument("-v", "--verbose", help="Determine the amount of information displayed.", action="store_true",
                     default=False)
    prs.add_argument("-o", "--old", help="Extract the old bitmap data found in the BMCache file.", action="store_true",
                     default=False)
    prs.add_argument("-b", "--bitmap", help="Provide a big bitmap aggregating all the tiles.", action="store_true",
                     default=False)
    prs.add_argument("-w", "--width",
                     help="Specify the number of tiles per line of the aggregated bitmap (default=64).", type=int,
                     default=64)
    prs.add_argument('-k', "--kape", help="Use this option to split out the different inputs into separate folders",
    ...
    for src in src_files:
        ...
        if bmcc.b_import(src):
            ...
            if (args.kape == True):
                destination = src.replace("\\", "_").replace("//", "_").replace(":", "_").replace(
                    "_AppData_Local_Microsoft_Terminal Server Client_Cache", "")
